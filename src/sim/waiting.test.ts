...
import { waitingGates } from "./waiting";
...
describe("waitingGates", () => {
  ...
  it("reports a null live margin when the buy side has zero local stock", () => {
  ...
  it("resolves multiple gates when several buy orders hold the same Stop (v1 atomic wait)", () => {
    ...
    expect(gates.map((g) => g.good).sort()).toEqual(["grain", "textiles"]);
  });
});

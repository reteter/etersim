...
/** One status word per ship, in priority order: "w przebudowie" (#276 — the
 *  Refit lock) beats everything else, since starting a Refit auto-suspends
 *  the ship's assignment (commands.ts) and a locked ship can't sail/trade
 *  regardless of what its Route thinks; then a suspended assignment beats
 ...
 *  (docked / underway / on route / suspended / czeka / w przebudowie) so the
 *  six values are mutually exclusive — `waiting` never coexists with
 *  `suspended` (`runRouteForShip` clears it on redirect, ADR-0007). */

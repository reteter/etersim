...
- **Cross-session state lives in the issue tracker, not in a document** (owner decision 2026-07-28). What carries between sessions: open issues (`gh issue list`), decisions waiting on the owner (`gh issue list --label needs:owner-decision`), and the owner-agreed order of work (milestone descriptions, `gh api repos/:owner/:repo/milestones`). `docs/HANDOFF.md` was retired the same day — a hand-maintained note has no mechanism keeping it true, and its predecessors drifted, went stale between owner requests, and outlived their own promises. **Project notes do not live in per-machine auto-memory either** — that channel is only for what is true of this machine and this owner.
...
5. **Feature branch + PR, conventional commits, `Closes #n`.** Before merge: tests, typecheck, lint, and the wave check at the change's tier (`docs/workflows/verification.md`). **The driver merges its own PRs once those gates are closed** (ADR-0010) — the owner keeps the final call on design and scope, not on the button. **Coder subagents never merge.** The undo half of the retired owner-merge gate is the mirror: `scripts/mirror-snapshot.ps1 -Epic E<n>` before an epic's first dispatch.
6. **Never act on `main`**: no commits, checkouts, resets. One documented exception: the session-close docs-only batch. In a worktree, address git as `git -C <worktree>`; verify with `git worktree list` (incident 0001).
...
- **Running-in, not sanding down** (owner framing, 2026-07-19). When a rule collides with the driving model's grain — it gets skipped under pressure, or obeyed in letter and missed in spirit — **reshape the rule, don't drill the model**. A rule that keeps being broken is usually the wrong shape: turn it into a command, a gate or a script rather than into sterner prose. (This repo's own evidence: the pre-work checklist grew a paragraph after each of five incidents and still dissolved under budget pressure — incident 0022 — until it became `npm run selfcheck`.)
...
- **The process constitution is edited only when the owner asks, in that session** — `CLAUDE.md`, `AGENTS.md`, `docs/workflows/*`, `docs/PROCESS.md`, `docs/personas/*`, `.claude/agents/*`, `.claude/hooks/*`, `.claude/settings.json`. Editing them is a decision, not a task: report what you would change and let the owner decide. Everything else stays freely editable — `CONTEXT.md` (glossary-first *requires* the entry before the identifier), `docs/specs/*`, `docs/PRD.md`, `docs/adr/*`, `docs/design-notes/*`, `docs/incidents/*` (never gate filing an incident). Enforced in Claude Code by `.claude/hooks/constitution-guard.sh`; written here because a hook is harness-specific and this rule is not. **This is also why the constitution carries no narrative:** a rule protected by a gate no longer has to argue for its own existence in order to survive a later model's tidying (owner decision 2026-07-28).
- Session start: `gh issue list`, then `gh issue list --label needs:owner-decision` (what is blocked on the owner), then the milestone descriptions (the agreed order), then prune merged branches. Session close: **leave the carry-over in the tracker** — file what was learned, close what was discharged, comment the newest acceptance criteria onto anything re-scoped. A thing worth telling the next session is worth an issue; if it does not deserve one, it does not deserve to be written down.

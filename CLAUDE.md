...
5. **Feature branch + PR, conventional commits, `Closes #n`.** Before merge: tests, typecheck, lint, and the wave check at the change's tier (WORKFLOW §Verification gates). **The owner merges — never merge your own PR without explicit owner consent.**
...
| **Orchestrator** | implementation: issues → packages → waves → integration | `docs/WORKFLOW.md` §Verification gates **with its subsections** (§Wave check, §Behavior-preserving exemption, §The review package, §Model ladder, §Fix loop, §E2E certification points, §Batching, §PR timing), plus §Pipeline steps 4–7, §Casting, §Documentation law, §Session rituals — **not the whole file**; `docs/personas/ORCHESTRATOR.md`; §Git & worktrees below; `docs/incidents/README.md` §Log; the epic spec + `docs/specs/README.md` |
...
- **Before declaring done**, walk the PR-template checklist (`.github/pull_request_template.md`) and close or explicitly flag every gate. Green tests and a commit are not "done" (incident 0004). Depth comes from one place: WORKFLOW §Verification gates. End the final report with each gate either **closed** (with evidence) or **OPEN** (with reason) — if a gate cannot be closed because a tool or skill is unavailable, say so; a silently skipped gate is worse than an open one.
...
- The wave check's **shape** comes from the tier table (`docs/WORKFLOW.md` §Verification gates) — tier 3 is ONE two-axis packaged subagent; the generic `/code-review` skill (two-agent fan-out) runs only on explicit owner request (incident 0016). Where repo docs say "/code-review" they mean the review gate, not that skill.

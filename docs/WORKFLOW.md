...
5. **Implementation** — branch `feat/<issue>-<slug>` (or `fix/`, `chore/`). Conventional commits (`feat:`, `fix:`, `chore:`, `docs:`, `test:`, `refactor:`). Sim code (`src/sim`) grows test-first (TDD, Vitest; default path — the evidence
   alternative lives in the CODER.md TDD line).
...
The coder's entire checklist lives in
...
Owner ruling 2026-07-19, #316. A wave whose `src/sim`
...
At every tier the package names the repo's scar tissue explicitly: the ACs,
...
The session driver (most expensive rung) composes packages, reads
...
Findings return to the *same* coder via resume (full transcript, zero
...
Affected specs per PR (coder-side). One full Playwright
...
Owner lock, 2026-07-15. No milestone closes on green
...
2–4 small same-area issues per coder package. **Prefer one PR for the
...
Coders push branches and report; **PRs open only after the wave check

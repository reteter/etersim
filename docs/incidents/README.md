...
| [0025](0025-worktree-isolation-not-provisioned-async-dispatch.md) | 2026-07-22 | Two async coder dispatches with `isolation: "worktree"` found no harness worktree pre-provisioned (s20) and each improvised the incident-0012-forbidden manual `git worktree add`; main stayed clean both times (coder discipline, not the mechanism). A same-session repro with the identical dispatch shape (async, `coder`, `isolation: "worktree"`) provisioned correctly — root cause unresolved, looks like a one-off rather than a structural driver. Watch for recurrence; capture the exact dispatch call shape if it happens again. |

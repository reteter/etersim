...
| [0025](0025-worktree-isolation-not-provisioned-async-dispatch.md) | 2026-07-22 | Two async coder dispatches with `isolation: "worktree"` found no harness worktree pre-provisioned (s20) and each improvised the incident-0012-forbidden manual `git worktree add`; main stayed clean both times (coder discipline, not the mechanism). **Root cause found s24:** `isolation: "worktree"` provisions a worktree only for **background** agents — a synchronous `run_in_background: false` coder silently shares the driver's main checkout. Controlled result (same coder/package/flag, foreground failed, background provisioned). Fix = dispatch-side rule "always background" in CLAUDE.md §Git + ORCHESTRATOR.md; coder stop-condition stays as backstop. |
| [0026](0026-cwd-drift-recurred-mid-session-now-mechanized.md) | 2026-07-22 | The 0008/0021 cwd-drift class recurred a third time, mid-session, minutes after 0008 had been read and cited — prose incidents don't fire at the moment of risk. A stray empty-content branch landed on origin from a bare `git checkout -b` run inside a stale worktree cwd; caught by a `git add` pathspec error, cleaned up. Mechanized instead of re-documented: `.claude/hooks/git-worktree-guard.sh` (PreToolUse on Bash) now blocks any bare, state-mutating git command whose resolved toplevel isn't the main repo root, unless it already carries `-C` or a self-contained `cd`. Needs one `/hooks` reload to activate in a session already running when the hook file was created. |
| [0027](0027-force-deleted-branches-under-owner-hold.md) | 2026-07-22 | Session-start "prune merged branches" force-deleted (`git branch -D`) two local branches that were **unmerged and under an explicit owner hold** (eval-1 quarantine) — because only the `MEMORY.md` index line was read, not the session-state body that carried the hold. Outcome Low (origin refs + closed PR #371 intact, local restored 1:1); class High (`-D` overrides git's native unmerged-delete refusal; `-d` would have refused). Same prose-doesn't-fire-at-risk shape as 0026. Mechanized: `.claude/hooks/git-branch-delete-guard.sh` (PreToolUse on Bash) returns **ask** (not deny — owner decides) for `git branch -D` and remote branch deletes; `git branch -d` and postmerge.ps1's PowerShell deletes pass. Needs one `/hooks` reload to activate mid-session. |
| [0028](0028-eval2-isolation-instrument-gaps-caught-mid-flight.md) | 2026-07-22 | Three eval-2 isolation/instrument gaps, each caught before it contaminated a measurement: (1) the `eval2-baseline` tip-only strip left the rubric reachable in git **history** (`git show <freeze-commit>`), and "temporal isolation is free" broke the moment the OS control ran first as arm 1 — its branches/PRs became a **reference solution on origin**; (2) the arm prompt said "read CLAUDE.md … don't read HANDOFF," but CLAUDE.md §Rules itself says "session start: read HANDOFF" — both solos followed the repo doc (reclassified as a shared instrument artifact, not an arm defect); (3) GPT/Codex auto-named Terra's branch `codex/…`, leaking authorship to the author-blind ruler. Fixes: **shallow single-branch clones** for every arm/ruler (`--depth` seals history, `--single-branch` seals other arms), strip/override CLAUDE.md's read-HANDOFF step, **author-neutral branch alias** before ruler dispatch. Outcome Low (none reached a measurement), class High. |
| [0029](0029-engineer-hat-worn-silently-unvalidated-feasibility.md) | 2026-07-22 | The **Engineer** altitude was worn without the ENGINEER.md-required explicit announcement during the E16/#390 Designer grills: two #390 forks were feasibility questions in disguise — net margin "derivable cleanly" (Q1) and ROI *velocity* "× loops-per-period" (Q2) — locked as clean Designer decisions carrying **unchecked** Engineer claims. Working-law 1 (a claim names its test, else it's an open question) was skipped. The `advisor`-before-writeup call did the Engineer feasibility pass that should have been conscious: the `dockingFee` `routeId` tag gives a *sum* not a loop *boundary*, and `ShipAssignment`/the Ledger carry no loop counter or loop-close event — so "per loop" was underivable without a second sim change, forcing corrections Q5 (tag) and Q6 (reframe velocity to per-**day**). Outcome Low (caught pre-spec/pre-cut), class Med (an underivable metric could have dispatched a doomed coder package — the "spec on an outgrown model" loss). Same soft-precondition-dissolves-under-flow shape as 0022/0021. Prevent: a feasibility-shaped grill fork is an **Engineer-hat trigger** — announce the hat + apply law 1 before the owner locks it (landed in ENGINEER.md §Invocation). Detect: the advisor-before-substantive-writeup habit. |

...
| [0030](0030-detector-merged-red-never-run.md) | 2026-07-28 | `check:triggers` has never exited 0 and sits in no gate, so the repo believed a law was mechanically enforced while nothing ran it. A tool that encodes a law is not the law being enforced: merge a detector green and wired to a gate in the same PR. |

...
| [E11 — Proving grounds](E11-proving-grounds.md) | **In progress** (M2 tooling), v1 slices 1–2 shipped (#232, #233): top-level `harness/` (own tsconfig project, run with `tsx`, outside the Vite bundle), the `Policy<M>` contract with `doNothing`/`gradientLoop` reference policies, the `advanceDays` seam in `src/sim/scenario.ts` (the E3 guardrail suites run on it), the Batch runner (`harness/batch.ts`), per-Run metrics derived purely from the Ledger + daily fleet snapshots (`harness/metrics.ts`), head-to-head policy comparison generalizing the #60 dominance guardrail (`harness/compare.ts`), and the `harness run` CLI writing per-Run JSONL Ledgers + a JSON/Markdown Batch report (`harness/cli.ts`, `harness/report.ts`). Outstanding: **#234** (runtime invariant assertions + anomaly-list population + `docs/experiments/` convention — this wave shipped the anomaly-list *plumbing* only, every Run already carrying its seed and replay command). | Headless evaluation harness: policy contract, `advanceDays` seam, Batch runner + `harness run` CLI, metrics/reports. v1 scope is Batch core + CLI; `harness play`/`replay` and an MCP adapter are v2-deferred in the spec itself. §Ledger schema binds the shipped `src/sim/ledger.ts` union — the harness serialises it, it does not define its own shape. |

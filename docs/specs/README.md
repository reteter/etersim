...
| [E11 — Proving grounds](E11-proving-grounds.md) | **In progress** (M2 tooling), v1 slice 1 shipped (#232): top-level `harness/` (own tsconfig project, run with `tsx`, outside the Vite bundle), the `Policy<M>` contract with `doNothing`/`gradientLoop` reference policies, and the `advanceDays` seam in `src/sim/scenario.ts` — the E3 guardrail suites run on it. Outstanding: **#233** (Batch runner + `harness run` CLI + metrics/reports), **#234** (runtime assertions + anomaly list + `docs/experiments/`). | Headless evaluation harness: policy contract, `advanceDays` seam, Batch runner + `harness run` CLI, metrics/reports. v1 scope is Batch core + CLI; `harness play`/`replay` and an MCP adapter are v2-deferred in the spec itself. §Ledger schema binds the shipped `src/sim/ledger.ts` union — the harness serialises it, it does not define its own. |

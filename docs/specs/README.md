...
| [E13 — Guild buildings](E13-guild-buildings.md) | **Open** (milestone #8) — not yet started; the E13.0 prerequisite shipped 2026-07-21, so it is now unblocked | Building permits gated by guild rank; the Granary (agrarian Storehouse variant) with store/withdraw Route orders; E9 construction machinery generalized to buildings. OQ8 is **resolved** (2026-07-21) and written into this spec's §Ledger: the Storehouse's netWorth value gets its own `buildingStoreValue` `NetWorthBreakdown` field, `SAVE_VERSION` bumps to 14 backfilling zero ([grill record](../design-notes/oq8-buildingStoreValue-grill-2026-07-21.md)) — the decision is in the spec; carrying it into #100's *code* is implementation work not yet started. The typed site-registry design in this file's §Tech is superseded by [ADR-0008](../adr/0008-one-goods-store.md) — see the file's own "§The site registry — superseded" section. |

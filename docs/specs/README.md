...
| [E16 — Workbench](E16-workbench.md) | **In progress** (M4). The 2026-07-29 **visual drift** finding is discharged: the spec now carries a **§Visual contract** section (written 2026-07-30 from a prototype, deliberately after the look rather than before it), and the board was rebuilt to it. Fan-out: **#393 withdrawn** — its surface no longer exists. Open riders: **#469** (iteration items + D3, which the prototype argues may need *revising*, not restyling), **#470** (`priceSnapshots` lost its last UI reader with the trend), **#471** (board density, since column pinning was removed rather than deferred), **#390** (the roster's simultaneous cross-Route view is gone with the tab). D7's context menu is held back on a popover component the repo lacks. | The Price Board becomes the game's workbench: Routes authored on it **port-centric** — as-built the gesture is a **good-cell radial menu** that places the Stop and attaches the order at once — Ships dispatched from it, one **market-quality signal** rendered as intensity across board / PortPanel shading / offer labels. **The Trasy tab is removed outright** (2026-07-30): the board owns the roster (Route tabs), the edit seam and the Route's operational controls (`RouteOpsStrip`), and `RoutesTab.tsx` is deleted. The Headquarters absorbs the retired **Księga** overlay as its default `Wartość firmy` tab, whose all-ships log reads one net line per day; the founding button moves to the top bar. The price trend and column pinning both **left the UI**. Market-free kinds (`deliver`/`store`/`withdraw`) are authored through the "więcej" drawer; cells carrying one are inert to the plain click. **UI + store only — no `src/sim`**, so file-disjoint from E11/E15. Route semantics stay frozen at E9/E9.1 (ADR-0007) — surface, not new automation. |

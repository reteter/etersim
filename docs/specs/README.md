...
| [E16 — Workbench](E16-workbench.md) | **In progress** (milestone M4) — enabler #392 + core authoring #394 (s24) + PortPanel shading #396 + runtime legibility #398 (s25, parallel #406 wave) shipped; fan-out remaining: #393 (roster) **gated by #404**, #395+#405 (density + authoring nits, batched, in flight s26), #397 (offer labels, ex-#227, solo — cross-cutting). Selector signature landed as-built `(ports)`, not spec's `(region, priceSnapshots)`. **Binary rendering ratified (#409 (a), 2026-07-28):** the selector grades `strong`/`mid`/`weak` (`NEAR_BEST_BAND = 0.08`) but every surface renders `strong` bright and `mid`/`weak` plain; §rendering-1/2 prose synced to match, with the third level left computed-but-undrawn so restoring it stays a rendering change. | The Price Board becomes the game's workbench: Routes authored on it **port-centric** (place port Stops, attach orders against live prices; good-centric is read-assist only, highlight-not-auto-wire), Ships dispatched from it, a single **market-quality signal** rendered across board / PortPanel action-shading / offer labels (#227), the Headquarters Trasy tab demoted to a read-only **Route-ribbon** roster with an edit-in-board seam, plus contextual-focus + column-pinning density tools and runtime sell-all legibility (cluster B symptom c). **UI + store only — no `src/sim` changes**, so file-disjoint from E11/E15 and parallelizable; priority slot an owner call. #376 is the grill+spec meta-ticket this spec fulfils (it closes on approval; impl lands as fresh E16 issues); #177 folds into the roster issue, #227 becomes the offer-labels issue. Route semantics stay frozen at E9/E9.1 (ADR-0007) — surface, not new automation. |

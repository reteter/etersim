...
| [E11 — Proving grounds](E11-proving-grounds.md) | **Open** (M2 tooling), v1 not started. | Headless evaluation harness: policy contract, `advanceDays` seam, Batch runner + `harness run` CLI, metrics/reports. v1 scope is Batch core + CLI; `harness play`/`replay` and an MCP adapter are v2-deferred in the spec itself. §Ledger schema binds the shipped `src/sim/ledger.ts` union — the harness serialises it, it does not define its own. |

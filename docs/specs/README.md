...
| [E14 — Shipyard & Refit](E14-shipyard-and-refit.md) | Shipped | Company-owned Shipyard building: ship commissioning as a construction site, Refit orders that change a ship's Hold. **Its three-step `deliver` chain predates E13's explicit addressing — read against `CONTEXT.md` before relying on it.** |
...
| [E15 — Processing](E15-processing.md) | **Open** (M3), not started; sequenced after E13 → E11. | Company-owned processing plants converting delivered inputs into processed goods (provisions, clearwood). Builds on E13.0's `GoodsStore`/`StorePolicy`; plant stores fold into the same `buildingStoreValue` field. Chain 3 (superconductor) deferred to the events+ice epic. **Known drift: the spec's §Addressing and §Testing contradict each other, and its `SAVE_VERSION` bump names v14, which E13 consumed — OQ8 settled v15.** |
| [E11 — Proving grounds](E11-proving-grounds.md) | **Open** (M2 tooling), v1 not started. | Headless evaluation harness: policy contract, `advanceDays` seam, Batch runner + `harness run` CLI, metrics/reports. v1 scope is Batch core + CLI; `harness play`/`replay` and an MCP adapter are v2-deferred in the spec itself. **Known drift: §Ledger schema describes a `LedgerEvent` union that `src/sim/ledger.ts` no longer has — refresh before #232/#233.** |

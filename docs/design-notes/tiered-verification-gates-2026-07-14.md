...
| 8. Batching | one issue per coder / batch | **2–4 small same-area issues per coder package; separate branch + PR per issue, each cut from main (no stacks for disjoint files)** | orientation amortizes across the batch; per-issue PRs keep `Closes #n` clean and avoid the stacked-PR procedure |

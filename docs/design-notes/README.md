...
| [design-surface-sweep](design-surface-sweep.md) | **LIVE** | The standing docs-vs-docs pass: reads the project's documents *against each other*, because every gate we run reads a diff and a contradiction between two idle documents has no diff to appear in. Opened s13 (2026-07-19) over the **whole** design surface (98 files, ~8.5k lines, plus 196 issue numbers the docs cite). Method: subject-keyed, not pairwise — grep each `CONTEXT.md` term and adjudicate the hits together, which leans on ubiquitous language being enforced here. Pass A **complete** (s13); Pass B opened s14, its rows ordered by whether `src/sim` can arbitrate rather than by glossary order. Two s14 lessons sit in the note: the 81 enumerable entries are a **subset** of the vocabulary, not the whole of it (F13 — `elasticity` carries two meanings and has no entry), and every corpus-wide count must be **anchored** against a known answer before it is trusted (F12 / incident 0020). Carries the binding rules (**HIST does not bind**; a LIVE doc citing HIST as current does), the owner's read-only-plus-routing finding policy, and the findings table. **This table is the resumable state** — take the topmost pending row in its Order column. |

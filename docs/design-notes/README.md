...
| [e13-0-shrink-measurement-2026-07-21](e13-0-shrink-measurement-2026-07-21.md) | **LIVE** | The interim measurement of the HANDOFF bet "E13.0 should make #100 smaller." Read from #100's own acceptance-criteria history (body → two amendments), not from LOC — because #100 is unimplemented, so the definitive shipped-diff measure is unavailable until **E13 close**. Finding: an entire guard sub-project (typed site registry + exhaustive switch + per-kind tests) was **deleted** from #100, and four abstractions (`GoodsStore`, `Transfer`, `accepts`/`StorePolicy`, `computeNetWorth`-over-`companyStores`) **moved into E13.0** — so #100's residual scope is *wiring, not inventing*. Direction of the bet holds on scope/risk; the note deliberately does **not** claim "confirmed" and carries the exact steps to close it definitively at E13 close. |
| [oq8-buildingStoreValue-grill-2026-07-21](oq8-buildingStoreValue-grill-2026-07-21.md) | HIST | Closes OQ8, the last open decision blocking #100 (E13): a building's standing goods store (Storehouse now, Plant at E15) gets its own `NetWorthBreakdown` field, `buildingStoreValue` — generic rather than `storehouseValue` so E15 reuses it without a second version bump — `SAVE_VERSION` 14, backfilled zero on older saves. Run in parallel with the #307 coder wave. All three citing specs updated same-commit; `docs/HANDOFF.md` queue item 3 deliberately left for the owner (ceremony-gated). |
| [parked-item-audit-2026-07-21](parked-item-audit-2026-07-21.md) | HIST | Closes #326: the trigger-is-a-promise law's own pre-law backlog. 79 anchored grep hits across 20 files, triaged; three genuine orphans (no issue, no milestone home) filed as #357 (route-conditionals / Grill cluster A), #358 (Grill cluster B), #359 (worldgen sole-producer-rim watch, deduplicated across two notes) with pointers written back. Settles the standing rule once: a grill tied to a named PRD milestone slot (the M4/M5/M6 grill briefs) needs no issue — the roadmap sweep is its tracker; a grill with no milestone slot does. `HANDOFF.md`'s quoted "parked-in-a-lot" line was already gone (#331 tsunami) — checked, nothing to strike. |
| [s14-law-automation-decision-2026-07-21](s14-law-automation-decision-2026-07-21.md) | **LIVE** (until implementation PR lands) | Decides #332: of the four s14 laws, two get a standing detector (`check-behavior-preserving.mjs` — hard-fails on added/removed test files, surfaces touched assertion lines for human review; `check-unpark-triggers.mjs` — the trigger-is-a-promise detector, exempting `grill-brief-m*-*.md` per #326's rule) and two are recorded as deliberately not automated (propagation — a documents-checked statement is gameable by construction; anchored counts — a session-time discipline, not something committed to git). Names, without answering, the bigger question the issue itself declines to settle: is `WORKFLOW.md` outgrowing what one reader can hold. |
